...
{
  public sealed class LISUBatteryComp : BaseComponent<BatteryComp>, IUIDataProvider, IEnergyProducer
  {
    ...
    private static void Register() => BaseComponent<BatteryComp>.AddComponentPrototype((IComponent) new LISUBatteryComp());

    protected override void OnConfig()
    {
      this.capacity = this.Config.GetInt("Capacity", 15000f);
      this.maxChargeRate = this.Config.GetFloat("ChargeRate", 150f);
      this.maxDischargeRate = this.Config.GetFloat("DischargeRate", 20f);
    }
